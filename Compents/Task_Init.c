...
TaskHandle_t KeySendTask_Handle; // 按键发送任务
...
void KeySendTask(void *pvParameters); // 按键发送任务
...
void Task_Init(void)
{
 ...
 // xTaskCreate(KeySendTask, "KeySendTask", 128, NULL, 2, &KeySendTask_Handle); // 创建按键发送任务
}
...
void Uart_Tx(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    case REMOTE:
      ...
      break;
    case AUTO:
      chassis.exp_vel.x = chassis_vel.Vx;
      chassis.exp_vel.y = chassis_vel.Vy;
      chassis.exp_vel.z = chassis_vel.Vw;
      break;
    case STOP:
    default:
      chassis.exp_vel.x = 0.0f;
      chassis.exp_vel.y = 0.0f;
