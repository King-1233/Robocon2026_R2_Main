...
void Quintic_GetFullState(QuinticParam_t* seg, uint32_t current_tick, TrajectoryState_t* out)
{
    ...
    out->pos = ((((seg->f * t + seg->e) * t + seg->d) * t + seg->c) * t + seg->b) * t + seg->a;//单位：米
    out->vel = (((5.0f * seg->f * t + 4.0f * seg->e) * t + 3.0f * seg->d) * t + 2.0f * seg->c) * t + seg->b;//单位：米/秒
    out->acc = ((20.0f * seg->f * t + 12.0f * seg->e) * t + 6.0f * seg->d) * t + 2.0f * seg->c;//单位：米/秒²
}
...
/**
 * @brief 快速S型曲线初始化
 * @param duration 期望到达时间
 */
void QuickS_SetTrajectory(QuinticParam_t *seg,
                          float p0, float pT, 
                          float duration, uint32_t current_tick)
{
    if (duration < 0.02f) duration = 0.02f; // 允许更高的控制频率
...
void QuickS_GetFullState(QuinticParam_t* seg, uint32_t current_tick, TrajectoryState_t* out)
{
    // 状态检查
    if (!seg->is_running || (current_tick < seg->start_time_ms)) {
        out->pos = seg->target_pos;
    ...
    // 预计算中间变量
    float dist = seg->target_pos - seg->start_pos;
    float invT = 1.0f / seg->T;
    float ratio = t * invT;
    const float PI2 = 6.2831853f;

    // 1. 位置计算 (S-curve Profile)
    // 相比五次多项式，正弦曲线在中间段的斜率（速度）更大，到达感更强
    out->pos = seg->start_pos + dist * (ratio - sinf(PI2 * ratio) / PI2);
    
    // 2. 速度计算 (v = d/dt pos)
    // v = (dist/T) * (1 - cos(2π*t/T))
    out->vel = (dist * invT) * (1.0f - cosf(PI2 * ratio));
    
    // 3. 加速度计算 (a = d/dt vel)
    // a = (2π*dist/T^2) * sin(2π*t/T)
    out->acc = (PI2 * dist * invT * invT) * sinf(PI2 * ratio);
}
